...
static word_t j_of(uint32_t i) { return (SEXT(BITS(i, 31, 31), 1) << 19) | (BITS(i, 19, 12) << 11) | (BITS(i, 20, 20) << 10) | BITS(i, 30, 21); }
...
static void decode_operand(Decode *s, word_t *dest, word_t *src1, word_t *src2, int type) {
  ...
  switch (type) {
    ...
    case TYPE_J: src1I(s->pc);   src2I(j_of(i)); destI(rd);  s->snpc = s->dnpc-4; break;
  }
}
...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = src1 + src2);
