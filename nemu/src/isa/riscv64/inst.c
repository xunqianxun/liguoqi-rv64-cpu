...
static word_t j_of(uint32_t i) { return (SEXT(BITS(i, 31, 31), 1) << 19) | (BITS(i, 19, 12) << 12) | (BITS(i, 20, 20) << 10) | BITS(i, 30, 21) << 1;}
...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, op2data = src2; printf("%lx",op2data); R(dest) = s->pc + 4; s->dnpc = (src1 + op2data) & 0xfffffffffffffffe);
