...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 000 ????? 01110 11", addiw  , I, src1 = ((src1 + src2) & 0x00000000ffffffff); if((src1 & 0x000000008000000) == 1) R(dest) = src1 | 0xffffffff00000000; else R(dest) = src1;);
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , R, R(dest) = src1 & src2);
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, if(src1 == src2) s->dnpc =s->pc + dest; else s->pc = s->pc;);
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if(src1 >= src2) s->dnpc =s->pc + dest; else s->pc = s->pc;);
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, if(src1 != src2) s->pc = s->pc;);//{ if((dest & 0x800) == 1) s->dnpc =s->pc - (~ dest +1); else s->dnpc = s->pc + dest;} else s->pc = s->pc;);
