...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(dest) = src2 + src2);
  ...
  INSTPAT("0000000 ????? ????? 000 ????? 01110 11", addw   , R, src1 = ((src1 + src2) & 0x00000000ffffffff); if((src1 & 0x000000080000000) == 0x000000080000000) R(dest) = src1 | 0xffffffff00000000; else R(dest) = src1;);
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, src1 = ((src1 + src2) & 0x00000000ffffffff); if((src1 & 0x000000080000000) == 0x000000080000000) R(dest) = src1 | 0xffffffff00000000; else R(dest) = src1;);
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, op1data = src1; op2data = src2; if(op1data >= op2data) { if((dest & 0x1000) == 0x1000) s->dnpc =s->pc - (~dest + 1); else s->dnpc =s->pc + dest;} else s->pc = s->pc;);
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, src1 = Mr(src1 + src2, 1); if((src1 & 0x0000000000000080) == 0x0000000000000080) R(dest) = (0xffffffffffffff00 | src1); else R(dest) = src1;);//switch ((src1 + src2) & 0b111){
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest) = Mr(src1 + src2, 1););//switch ((src1 + src2) & 0b111){
  ...
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, src1 = Mr(src1 + src2, 2); if((src1 & 0x0000000000008000) == 0x0000000000008000) R(dest) = 0xffffffffffff0000 | src1 ; else R(dest) = src1;);//op1data = src1 + src2; switch (op1data & 0b110){
  ...
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(dest) = Mr(src1 + src2, 2););  //switch ((src1 + src2) & 0b110){
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, src1 = (Mr(src1 + src2, 4)); if((src1 & 0x0000000080000000) == 0x0000000080000000) R(dest) = (0xffffffff00000000 | src1) ; else R(dest) = src1;);//op1data = src1 + src2; switch (op1data & 0b100){
  ...
  INSTPAT("??????? ????? ????? 110 ????? 00000 11", lwu    , I, R(dest) = Mr(src1 + src2, 4);); //switch ((src1 + src2) & 0b100){
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh    , R, op1data = src1; op2data = src2; R(dest) = (op1data * op2data) & 0xffffffff00000000);
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu  , R, op1data = src1; R(dest) = (op1data * src2) & 0xffffffff00000000);
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu   , R, R(dest) = (src1 * src2) & 0xffffffff00000000);
